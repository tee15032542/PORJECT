  ...
  setup() {
    ...
    async function getLogin() {
      ...
      if (stateLogin == 'user') {
        ...
        axios
          .get('http://localhost:8081/getUserId/id=' + personalId)
          .then(res => {
            console.log(res.data)
            if (res.data != null) {
              store.dispatch('getUserProfile', res.data['0'])
              axios.get('http://localhost:8081/getPolygonByUser/id=' + personalId).then(res => {
                userGeometry.value = res.data['0'].geometry_data
                console.log(userGeometry.value)
                store.dispatch('getUserGeometry', userGeometry.value)
                         console.log(store.state.userGeometry)
              })
         ...
         axios
          .get('http://localhost:8081/getAllPolygon' )
          .then(res => {
            // console.log(res.data)
            if (res.data != null) {
              let allPolygon = res.data
              console.log(allPolygon)
              store.dispatch('getUserGeometry', allPolygon)
              router.push({ path: '/home' })
